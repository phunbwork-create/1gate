...
import { CreditCard, Plus, Search, Loader2, ChevronLeft, ChevronRight, Eye, SlidersHorizontal, X } from "lucide-react"
            ...
            {loading ? (
              ...
              return (
                <TableRow key={r.id} className="cursor-pointer hover:bg-muted/50"
                  ...
                  <TableCell>
                    {scenario && (
                      <Badge variant="outline" className={`text-xs ${scenario.color}`}>{scenario.label}</Badge>
                    )}
                  </TableCell>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" disabled={pagination.page <= 1}
              onClick={() => setPage(pagination.page - 1)}><ChevronLeft className="h-4 w-4" /></Button>
            <Button variant="outline" size="sm" disabled={pagination.page >= pagination.totalPages}
              onClick={() => setPage(pagination.page + 1)}><ChevronRight className="h-4 w-4" /></Button>
          </div>
        </div>
      )}
    </div>
