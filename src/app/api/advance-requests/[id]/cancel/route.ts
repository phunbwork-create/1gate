...
// ─── POST /api/advance-requests/[id]/cancel ───────────────────────────────────
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const result = await requireRole("ChiefAccountant", "Admin")
  ...
  try {
    ...
    return success({ message: "Đã hủy đề nghị tạm ứng" })
  } catch (error) {
    console.error("POST /api/advance-requests/[id]/cancel error:", error)
    return serverError()
  }
}
